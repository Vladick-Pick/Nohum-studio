# NoHum v1.8 Single-Import MCP and Tool Access Matrix
...
- The root package imports the org library in one pass, but only bootstrap and
  active day-1 runtime agents receive live work immediately.
...
| Research Lead | Hypothesis Funnel issues, Idea Cards, `Research Registry`, `Decision Memory`, queue package, eval export, evidence registry | paperclip, paperclip-knowledge, browser research, optional memory | `BRAVE_API_KEY` | No build or release writes; No bypass of Gate A evidence; No shadow history outside canonical artifacts | Existing live core slug should upgrade in place. |
| Idea Scout | TrustMRR sourcing batches, TrustMRR API, SimilarWeb enrichment, shortlist notes, current `Decision Memory` snapshot | paperclip, paperclip-knowledge, TrustMRR API, Apify SimilarWeb | `TRUSTMRR_API_KEY`, `APIFY_TOKEN` | No intake creation; No specialist launch; No final research verdict; No new `research_case_id` assignment | Pause after import until TrustMRR and SimilarWeb secrets are wired. |
...
| Product Bet Compiler | Frozen Idea Card, Gate A decision, Product Bet Card, validation risk map | paperclip, paperclip-knowledge, optional browser research | host-managed model auth | No pre-Gate-A product bet work; No edits to frozen Idea Card; No Gate B approval | Imported with company but dormant until CEO creates Product Bet Validation Sprint. Writes product identity, ICP, workflow, product-shape, and validation-risk sections. |
| Competitor Deep Dive Analyst | Competitor sites, signup/onboarding flows, pricing pages, screenshot notes, Product Bet Card competitor section | paperclip-knowledge, browser/web research, optional Playwright/browser automation | optional `BRAVE_API_KEY`, optional `APIFY_TOKEN` | No prohibited scraping; No claims without source refs; No invented onboarding facts when signup is blocked | Imported dormant. Activate only inside Product Bet Validation Sprint with allowed external actions from Gate A. |
| Economics Modeler | Pricing refs, financial model template, unit-economics notes, sensitivity model | paperclip-knowledge, spreadsheet/calculator-style modeling, browser pricing research | host-managed model auth | No finance certainty claims; No hidden assumptions; No build recommendation without break-even and $5k path scenarios | Imported dormant. Writes conservative/base/aggressive financial model and kill thresholds. |
| Offer & Positioning Strategist | Competitor deep dive, Product Bet Card, offer brief, battlecard | paperclip-knowledge, browser research | host-managed model auth | No unsupported superiority claims; No promises outside Gate A constraints or forbidden claims | Imported dormant. Writes USP, value prop, pricing frame, objections, and claim safety checks. |
| Organic Traffic Strategist | Search intents, directories, communities, GitHub/marketplace patterns, traffic strategy template | paperclip-knowledge, browser research, optional GitHub read | optional `BRAVE_API_KEY` | No spam; No fake endorsements; No public posting without approval | Imported dormant. Defines free traffic wedge and minimum viable distribution test before Gate B. |
| Pre-Market Autoreasoner | Product Bet revision, competitor pack, frozen research refs, autoreason report | internal model calls/subroles via runtime adapter; paperclip-knowledge | host-managed model auth | No external actions; No treating synthetic audience as market proof; No modifying immutable context | Imported dormant. Runs bounded critique/variant/judge loop and emits revision/fork candidates. |
| Landing Surface Builder | Offer brief, organic traffic brief, surface pack, static test surface notes | paperclip-knowledge, browser QA, optional repo/static-site tooling only when approved | host-managed model auth | No production product build; No public deploy without approval; No unsupported claims in landing copy | Imported dormant. Produces test GTM surface pack and coordinates claims/browser QA. |
| Product Bet Measurement Specialist | Measurement plan, analytics events, UTM policy, dashboard refs, QA notes | paperclip-knowledge, analytics read/setup when approved, browser QA | `ANALYTICS_API_KEY` when live testing is approved | No attribution certainty from weak instrumentation; No live metric interpretation before event QA | Imported dormant. Owns pre-Gate-B measurement plan and test instrumentation readiness. |
| Evidence Router | Evidence events, test results, decision rules, Gate B recommendation | paperclip, paperclip-knowledge, analytics read when approved | `ANALYTICS_API_KEY` when live testing is approved | No Gate B approval; No changing evidence thresholds after results; No evidence without source refs | Imported dormant. Turns internal/external evidence into build/revise/test_more/kill recommendation. |
